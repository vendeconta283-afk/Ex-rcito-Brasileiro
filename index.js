...
async function setRankNoGrupo(robloxUserId, novoRank) {
    const cookie = process.env.ROBLOX_COOKIE;
    if (!cookie) throw new Error('Cookie Roblox não configurado.');

    // 1. Obter o token XSRF
    let xsrfToken;
    try {
        ...
        if (!xsrfToken) throw new Error('Token XSRF não encontrado na resposta.');
    } catch (err) {
        throw new Error(`Falha ao obter token XSRF: ${err.message}`);
    }

    // 2. Fazer a requisição de alteração de rank COM o token XSRF
    const res = await fetch(`https://groups.roblox.com/v1/groups/${config.GRUPO_ID}/users/${robloxUserId}`, {
        ...
        body: JSON.stringify({ roleId: novoRank })
    });

    if (!res.ok) {
        const errText = await res.text();
        throw new Error(`Erro na API Roblox: ${res.status} ${errText}`);
    }
    return true;
}
...
client.once('ready', async () => {
    ...
    await client.application.commands.set([
        ...
        promoverCommand, rebaixarCommand
    ]);
...
async function handleTicket(interaction) {
    await interaction.reply({
        content: '🎫 Um ticket será criado em breve. Descreva sua dúvida e um membro da equipe entrará em contato.',
        ephemeral: true
    });
}
...
async function handlePromocao(interaction, tipo) {
    ...
    // 1. Verifica permissão do autor
    if (!await isAspiranteOrHigher(interaction.member)) {
    ...
    // 3. Obtém cargo desejado
    const nomeCargo = interaction.options.getString('cargo');
    const novoRank = config.RANK_POR_NOME[nomeCargo];
    if (novoRank === undefined) {
        return interaction.editReply(`❌ Cargo "${nomeCargo}" não encontrado. Use um nome exato, como "[REC] Recruta".`);
    }

    // 4. Executa
    try {
        await setRankNoGrupo(targetRobloxId, novoRank);
