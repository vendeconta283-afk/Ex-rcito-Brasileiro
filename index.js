...
// Retorna o rank (número) do autor no grupo
...
// Retorna o rank (número) de um usuário pelo ID do Discord
...
async function setRankNoGrupo(robloxUserId, nomeCargo) {
    ...
    // 2. Buscar a lista de cargos do grupo
    let roles;
    try {
    ...
    // 3. Encontrar o cargo pelo nome exato
...
const comprarPatenteCommand = new SlashCommandBuilder()
    ...
    .setDescription('Gera a tabela de compra de patentes')
    .setDefaultMemberPermissions(PermissionsBitField.Flags.ManageGuild);

const girarCommand = new SlashCommandBuilder()
    .setName('girar')
    .setDescription('Gire a roleta e ganhe prêmios!')
    .addStringOption(option =>
        option.setName('tipo')
            .setDescription('Tipo de roleta')
            .setRequired(true)
            .addChoices(
                { name: 'VIP', value: 'vip' },
...
client.once('ready', async () => {
    ...
    await client.application.commands.set([
        ...
        promoverCommand, rebaixarCommand, comprarPatenteCommand, girarCommand
    ]);
...
client.on('interactionCreate', async interaction => {
    try {
        if (interaction.isCommand()) {
            ...
            } else if (interaction.commandName === 'girar') {
            ...
            if (interaction.customId === 'btn_vincular') await startVerification(interaction);
                ...
                // Cria um ticket individual (sem categoria)
...
async function criarTicket(interaction) {
    ...
    // Cria um canal de texto privado (sem categoria)
    const canal = await guild.channels.create({
        ...
        permissionOverwrites: [
            ...
            // Adicione aqui IDs de cargos de staff que podem ver tickets (ex: "Administrador")
            { id: guild.roles.cache.find(r => r.name === 'Administrador')?.id, allow: [PermissionsBitField.Flags.ViewChannel] },
            { id: guild.roles.cache.find(r => r.name === 'Staff')?.id, allow: [PermissionsBitField.Flags.ViewChannel] },
            { id: guild.roles.cache.find(r => r.name === '[MOD] Moderador')?.id, allow: [PermissionsBitField.Flags.ViewChannel] },
        ]
    });
...
async function handleTabelaPatentes(interaction) {
    const embed = new EmbedBuilder()
        ...
        .setDescription('Adquira sua patente através de tickets! (Preços fictícios – configure no código)')
        .addFields(
...
async function handleGirar(interaction) {
    ...
    const premiosVip = ['VIP por 1 dia', 'VIP por 3 dias', 'VIP por 7 dias', 'VIP Permanente'];
    const premiosDinheiro = ['R$ 500', 'R$ 1.000', 'R$ 5.000', 'R$ 10.000'];
    const premiosXp = ['100 XP', '500 XP', '1.000 XP', '5.000 XP'];
    const premiosAleatorios = [...premiosVip, ...premiosDinheiro, ...premiosXp];

    let premio;
    if (tipo === 'vip') premio = premiosVip[Math.floor(Math.random() * premiosVip.length)];
    else if (tipo === 'dinheiro') premio = premiosDinheiro[Math.floor(Math.random() * premiosDinheiro.length)];
    else if (tipo === 'xp') premio = premiosXp[Math.floor(Math.random() * premiosXp.length)];
    else premio = premiosAleatorios[Math.floor(Math.random() * premiosAleatorios.length)];

    const embed = new EmbedBuilder()
        .setTitle('🎰 ROLETA DE PRÊMIOS')
        .setDescription(`${user} girou a roleta e ganhou:`)
        .addFields({ name: '🏆 Prêmio', value: `**${premio}**`, inline: false })
        .setColor(0x00ff00);
...
async function handlePromocao(interaction, tipo) {
    ...
    // Obtém o rank correspondente ao nome do cargo (para validar hierarquia)
    let cargoRank = 0;
    ...
    // Não pode promover alguém de rank maior ou igual ao seu
    ...
    // Só pode promover até o cargo imediatamente abaixo do seu
