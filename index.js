...
// ================= PROMOÇÃO / REBAIXAMENTO (COM BUSCA AUTOMÁTICA DE ID) =================
async function handlePromocao(interaction, tipo) {
    ...
    // 3. Obtém nome do cargo (já é a string exata)
    const nomeCargo = interaction.options.getString('cargo');
    if (!nomeCargo) {
        return interaction.editReply('❌ Cargo não especificado.');
    }

    // 4. Executa (setRankNoGrupo agora busca o ID automaticamente)
    try {
