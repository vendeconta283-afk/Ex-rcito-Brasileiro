...
async function getXSRFToken(cookie) {
    if (cachedXsrf && Date.now() < xsrfExpiry) return cachedXsrf;
    try {
        const res = await fetch('https://auth.roblox.com/v2/logout', {
            method: 'POST',
            headers: { 'Cookie': `.ROBLOSECURITY=${cookie}` }
        });
        cachedXsrf = res.headers.get('x-csrf-token');
        xsrfExpiry = Date.now() + 4 * 60 * 1000; // 4 minutos
        if (!cachedXsrf) throw new Error('Token XSRF não obtido. O cookie pode ser inválido.');
        return cachedXsrf;
    } catch (err) {
        throw new Error(`Falha ao obter XSRF: ${err.message}`);
    }
}
...
async function sendGameCommand(topic, data) {
    ...
    if (!cookie || !universeId) throw new Error('Variáveis ROBLOX_COOKIE/UNIVERSE_ID ausentes.');
    const xsrf = await getXSRFToken(cookie);
...
async function getUserInventory(userId, assetId) {
    ...
    if (!res.ok) {
        const err = await res.text();
        throw new Error(`Erro ao consultar inventário: ${res.status} ${err}`);
    }
    const data = await res.json();
